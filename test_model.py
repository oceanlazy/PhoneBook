...
class TestAdder(unittest.TestCase):
    def test_add_check(self):
        run = model.Model(LocalDataManager()).add_check('102')
        self.assertTrue(run)

    def test_create_contact(self):
        run = model.Model(LocalDataManager()).create_contact('Foo', 'Bar', '777')
    ...
    def test_update_contact(self):
        run = model.Model(LocalDataManager()).update_contact(1, 'Foo', 'Bar', '777')
        res = 'Contact successfully updated.\nName: Foo Bar Phone: 777'
        self.assertEqual(run, res)

    def test_delete_contact(self):
        run = model.Model(LocalDataManager()).delete_contact(1)
